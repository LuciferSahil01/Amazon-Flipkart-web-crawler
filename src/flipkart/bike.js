...
setInterval(function () {
  ...
  async function main() {
    ...
    await page.goto(
      "https://www.flipkart.com/search?q=bike+tyre&as=on&as-show=on&otracker=AS_Query_HistoryAutoSuggest_1_4_na_na_na&otracker1=AS_Query_HistoryAutoSuggest_1_4_na_na_na&as-pos=1&as-type=HISTORY&suggestionId=bike+tyre&requestId=53aa0c8e-3dd0-4a12-9dc4-1d99dc4bbad9&as-backfill=on",
      {
        timeout: 0,
    ...
    for (let i = 0; i < M; i++) {
      ...
      const productData = await page.evaluate(async (data) => {
        ...
        //document.querySelectorAll("classtitle")
        ...
        items.forEach((item, index) => {
          ...
          //Getting title data from Selector
          const title =
          ...
          const mrp =
            ...
            item.querySelector("div._3I9_wc").innerText.replace("₹", "").replace(",", "");
      ...
      (async () => {
        ...
        try {
          ...
          const flipkart_biketyreCollection = await client
            .db()
            .collection("flipkart_biketyreCollection");
          ...
          flipkart_biketyreCollection.bulkWrite(Update).catch((e) => {
            console.log(e);
          });
        } catch (e) {
